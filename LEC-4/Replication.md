...
lab 1 workers已经是容错的，但是 master 存在单点问题;万一失败了我们是否允许有两个master吗？
worker list;哪些工作完成;哪个工人闲着;TCP连接状态;程序计数器.
...
* State transfer
  * master 副本执行该服务
  * 主要将新状态发送到备份
* Replicated状态机（Replicated state machine）
  * 所有副本都执行所有操作
  * 如果启动状态相同，
      * 相同的操作，
    * 同样的顺序，
...
非常具有想法的机制(Very ambitious system)：
...
概观[图：应用程序，O / S，下面的VM-FT]
  
  * 两台机器，master和备用; 和其他机器
  * 两个网络：客户端到服务器，记录通道
  * 共享磁盘用于持久存储
  * 在“锁定步骤”与master备份
    * master将所有输入发送到备份
...
挑战：
  ...
  * 使它看起来像一个可靠的服务器
    * 如果master的失败和副本接管，外界会看到什么？
      * 如果在向客户端发送响应之前或之后master失败
      * 客户端的请求可能会丢失吗？执行两次？
    * master什么时候向客户发送回复？
  * 如何避免两个master？（“脑裂”）
    * 如果记录通道中断怎么办？
...
挑战2解决方案：共享磁盘
  ...
  * 理论的解决方案：假设共享磁盘
    ...
    * 在磁盘上备份原子性测试和设置变量
      * 如果设置，master仍然存在。自杀
      * 如果没有设置，master死亡。成为master
      * 如果是master，请从检查点创建新的备份
        * 使用VMotion
...
共享存储是单点故障
  ...
  确定纸张的设置，但是对于地理复制服务
    地震不能生存
```
问：为什么共享磁盘上没有master记录日志，那么备份怎么重放日志？
  优点：无需登录通道，无FT协议等
...
概要：
  ...
  * 如何处理裂脑？
  * 如何获得更好的表现？
    * 使用更高级复制状态机的master复制键/值操作，如put和get
